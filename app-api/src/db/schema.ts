...
export const scrapeTriggerEnum = pgEnum("scrape_trigger", [
  "scheduled",
  "manual",
  "backfill",
]);
...
export const jobs = pgTable(
  ...
  (t) => [
    ...
    index("idx_jobs_feed")
      .on(t.source, t.isActive, t.isDeleted, t.postedAt),

    // Location filter
    index("idx_jobs_city")
      .on(t.city, t.isActive, t.isDeleted, t.postedAt),

    // Job type filter
    index("idx_jobs_job_type")
      .on(t.jobType, t.isActive, t.isDeleted, t.postedAt),

    // Work mode filter
    index("idx_jobs_work_mode")
      .on(t.workMode, t.isActive, t.isDeleted, t.postedAt),

    // Compound multi-filter (source + city + jobType)
    index("idx_jobs_compound")
      .on(t.source, t.city, t.jobType, t.isActive, t.isDeleted, t.postedAt),

    // Scraper re-confirmation / expiry job
    index("idx_jobs_last_seen")
    ...
    index("idx_jobs_company")
      .on(t.companyNormalized, t.isActive, t.isDeleted),
...
export const savedJobs = pgTable(
  ...
  (t) => [
    ...
    index("idx_saved_jobs_clerk")
      .on(t.clerkId, t.isDeleted, t.updatedAt),

    // Status filter
    index("idx_saved_jobs_clerk_status")
      .on(t.clerkId, t.status, t.isDeleted),

    // FK cascade support
    index("idx_saved_jobs_job_id")
      .on(t.jobId),
  ]
);
...
export const savedJobStatusHistory = pgTable(
  ...
  {
    id:            uuid("id").primaryKey().defaultRandom(),
    savedJobId:    uuid("saved_job_id").notNull().references(() => savedJobs.id, { onDelete: "cascade" }),
  ...
  (t) => [
    ...
    index("idx_status_history_saved_job")
      .on(t.savedJobId, t.createdAt),

    // All activity for a user
    index("idx_status_history_clerk")
      .on(t.clerkId, t.createdAt),
  ]
);
...
export const scrapeLogs = pgTable(
  ...
  (t) => [
    ...
    index("idx_scrape_logs_source")
      .on(t.source, t.startedAt),
