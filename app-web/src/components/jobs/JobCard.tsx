...
export function JobCard({ job, active, saved, onClick, onSave }: JobCardProps) {
  ...
  return (
    <div
      onClick={onClick}
      className={cn(
        "w-full text-left rounded-[14px] border px-4 py-4 transition-all duration-150 group cursor-pointer",
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3 mb-2">
        <div className="flex items-center gap-2.5 min-w-0">
          {/* Company initial */}
          <div className="w-8 h-8 rounded-[8px] bg-muted border border-border flex items-center justify-center shrink-0">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {job.salary && (
          <>
            <span className="text-muted-foreground/30 text-[11px]">·</span>
            <span className="text-[11px] font-medium text-foreground/70">{job.salary}</span>
          </>
        )}

        <span className="ml-auto text-[10px] text-muted-foreground/40">{job.postedAt}</span>
      </div>
    </div>
  );
}
