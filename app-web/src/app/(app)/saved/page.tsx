      ...
      <main className="flex-1 flex flex-col">
        ...
        <div className="px-4 lg:px-6 py-4 flex flex-col gap-3">
          {Array.from({ length: 4 }).map((_, i) => (
            <div
              key={i}
              className="rounded-[10px] border border-border p-4 flex flex-col gap-3 animate-pulse"
            >
              <div className="flex gap-2">
                <div className="w-7 h-7 rounded-[6px] bg-muted" />
  ...
  if (savedJobs.length === 0) {
    return (
      <main className="flex-1 flex flex-col items-center justify-center gap-4 p-8 text-center">
        <div className="w-14 h-14 rounded-[10px] bg-muted border border-border flex items-center justify-center">
      ...
      <div className="flex flex-col flex-1 min-w-0 overflow-y-auto">
        <div className="px-4 lg:px-6 pt-5 pb-4 border-b border-border shrink-0">
          ...
          <p className="text-xs text-muted-foreground/50 mt-0.5">
            {savedJobs.length} job{savedJobs.length !== 1 ? "s" : ""} shortlisted · click to preview
            · track when ready to apply
          </p>
        </div>
        ...
        {staleCount > 0 && (
          <div className="mx-4 lg:mx-6 mt-3 flex items-center gap-2 px-3 py-2 rounded-[10px] bg-amber-500/8 border border-amber-500/20">
            ...
            <p className="text-[11px] text-amber-600 dark:text-amber-400">
              {staleCount} job{staleCount !== 1 ? "s have" : " has"} been sitting here for over 3
              days — apply or clear them out.
            </p>
          </div>
        )}
          ...
          {savedJobs.map(({ savedJob, job }) => {
            ...
            return (
              <div
                ...
                className={cn(
                  "rounded-[10px] border bg-card p-4 flex flex-col gap-3 cursor-pointer transition-colors",
                  isActive
              ...
              >
                <div className="flex items-center gap-2">
                  <div className="w-7 h-7 rounded-[6px] bg-muted border border-border flex items-center justify-center text-xs font-semibold text-foreground shrink-0">
                    {job.company[0]}
                  </div>
                  ...
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      mutations.remove(savedJob.id);
                      if (selectedJob?.id === job.id) setSelectedJob(null);
                    }}
                    className="w-7 h-7 rounded-[6px] flex items-center justify-center text-muted-foreground/30 hover:text-destructive hover:bg-destructive/10 transition-colors shrink-0"
                  >
                ...
                <div className="flex items-center justify-between pt-1 border-t border-border/40">
                  {!stale ? (
                    <span className="text-[11px] text-muted-foreground/40">
                      Saved {timeAgo(savedJob.createdAt)}
                    </span>
                  ) : (
                    <span />
                  )}
                  {tracked ? (
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        mutations.updateStatus(savedJob.id, "applied");
                      }}
                      className="flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-[6px] bg-foreground text-background hover:bg-foreground/80 transition-colors shrink-0"
                    >
      ...
      {selectedJob && (
        <JobDetail
          ...
          onSave={() => {
            ...
            if (match) {
              mutations.remove(match.savedJob.id);
              setSelectedJob(null);
            }
          }}
        />
